...
client.on('message', async message => {
   ...
   if(message.content.startsWith("$setadLeave")) {
             
    let filter = m => m.author.id === message.author.id;
    let thisMessage;
    let thisFalse;
 
    if(!message.member.hasPermission("MANAGE_GUILD")) return message.channel.send('').then(msg => {
       msg.delete(4500);
       message.delete(4500);
    });
            ...
            msg.edit(':scroll: **| اكتب اسم الروم الان... :pencil2: **').then(msg => {
                ...
                .then(collected => {
                    ...
                    msg.edit('✅ **| تم الاعداد بنجاح...  **').then(msg => {
                      ...
                      .setTitle('Done set Your Leave Channel')
                      .addField(``)
                      .addField(``)
...
let embed = new Discord.RichEmbed()
.setTitle('**Welcome Added To Your Server**')
.setThumbnail(message.author.avatarURL)
.setFooter(`${client.user.username}`)
message.channel.sendEmbed(embed)
welcome[message.guild.id] = {
...
if (err) console.error(err)
    ...
    if(message.content.startsWith("toggldsfdeLeave")) {
        if(!message.channel.guild) return message.reply('**This Command Only For Servers**');
        if(!message.member.hasPermission('MANAGE_GUILD')) return message.channel.send('');
...
client.on('message', message => {
    ...
    if(message.content.startsWith("$stwelcom")) {
        if(!message.channel.guild) return message.reply('**This Command Only For Servers**');
        if(!message.member.hasPermission('MANAGE_GUILD')) return message.channel.send('' );
        if(!welcome[message.guild.id]) welcome[message.guild.id] = {
          onoff: 'on'
    ...
    if(message.content.startsWith("toggdsdfleDmwelcome")) {
        if(!message.channel.guild) return message.reply('**This Command Only For Servers**');
        ...
        client.on('message', message => {
            ...
            if(message.content.startsWith("$setinvite")) {
                if(!message.channel.guild) return message.reply('**This Command Only For Servers**');
                if(!message.member.hasPermission('MANAGE_GUILD')) return message.channel.send('' );
                if(!welcome[message.guild.id]) welcome[message.guild.id] = {
                  by: 'on'
...
client.on("guildMemberAdd", member => {
    ...
    let memberavatar = member.user.avatarURL
      ...
      if(welcomer) {
        ...
        .addField('Joind discord in : ',`${moment(member.user.createdAt).format('D/M/YYYY h:mm a')} **\n** \`${moment(member.user.createdAt).fromNow()}\``,false)
...
client.on('guildMemberAdd', member => {
  ...
  member.guild.fetchInvites().then(guildInvites => {
    ...
    if(!logChannel) return;
      setTimeout(() => {
    logChannel.send(`**
   • Welcome To Server ${member.guild.name} • :two_hearts: 
   • User : ${member} • 
   • Invited By  : <@${inviter.id}> • 
   • Thanks You For Server Joint • :heart: :two_hearts: **
`);{}
  },2000)
  fs.writeFile("./welcome.json", JSON.stringify(welcome), (err) => {
    if (err) console.error(err)
...
client.login('NzI3MTg5OTE0NDY2MTIzODM4.XvoOYg.rht5Ic5PanUMCESEoZjwg7gXBiM');
